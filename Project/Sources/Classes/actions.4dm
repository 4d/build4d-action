...
Function _signMacOS($baseFolder : 4D:C1709.Folder) : Object
	...
	// Add deep signing option
	If (Bool:C1537(This:C1470.config.signDeep))
		$cmdSuffix:=$cmdSuffix+" --deep"
	End if 
	
	// Add timestamp option  
	If (Bool:C1537(This:C1470.config.signTimestamp))
		$cmdSuffix:=$cmdSuffix+" --timestamp"
	End if 
	
	// Add custom signing options
	If (Length:C16(String:C10(This:C1470.config.signOptions))>0)
		$cmdSuffix:=$cmdSuffix+" "+String:C10(This:C1470.config.signOptions)
	End if 
...
Function _signWindows($baseFolder : 4D:C1709.Folder) : Object
	...
	Storage:C1525.github.info("Starting Windows code signing")
	
	// Check if we have Windows signing configuration
	var $hasCertificate : Boolean
	$hasCertificate:=(Length:C16(String:C10(This:C1470.config.signWindowsCertificate))>0) || (Length:C16(String:C10(This:C1470.config.signWindowsThumbprint))>0)
	
	If (Not:C34($hasCertificate))
		Storage:C1525.github.error("No Windows signing certificate or thumbprint specified")
		return New object:C1471("success"; False:C215; "errors"; New collection:C1472("No Windows signing certificate or thumbprint specified"))
	End if 
	...
	If (Length:C16(String:C10(This:C1470.config.signWindowsThumbprint))>0)
		$cmd:=$cmd+" /sha1 \""+String:C10(This:C1470.config.signWindowsThumbprint)+"\""
		$cmd:=$cmd+" /s \""+String:C10(This:C1470.config.signWindowsStore)+"\""
	Else 
		If (Length:C16(String:C10(This:C1470.config.signWindowsCertificate))>0)
			$cmd:=$cmd+" /f \""+String:C10(This:C1470.config.signWindowsCertificate)+"\""
		End if 
	End if 
	
	// Add timestamp URL if provided
	If (Length:C16(String:C10(This:C1470.config.signTimestampUrl))>0)
		$cmd:=$cmd+" /tr \""+String:C10(This:C1470.config.signTimestampUrl)+"\""
		$cmd:=$cmd+" /td sha256"
	End if 
	...
	If (Length:C16(String:C10(This:C1470.config.signOptions))>0)
		...
		$signOptions:=JSON Parse:C1218(String:C10(This:C1470.config.signOptions))
		If ($signOptions#Null:C1517)
			// Add Windows signing options inline
			var $key : Text
			For each ($key; $signOptions)
				Case of 
					: ($key="description") && (Length:C16(String:C10($signOptions[$key]))>0)
						$cmd:=$cmd+" /d \""+String:C10($signOptions[$key])+"\""
					: ($key="url") && (Length:C16(String:C10($signOptions[$key]))>0)
						$cmd:=$cmd+" /du \""+String:C10($signOptions[$key])+"\""
					: ($key="algorithm") && (Length:C16(String:C10($signOptions[$key]))>0)
						$cmd:=$cmd+" /fd \""+String:C10($signOptions[$key])+"\""
					Else 
						Storage:C1525.github.debug("Unknown Windows signing option: "+$key)
				End case 
			End for each 
		End if 
	End if 
	...
	If ((This:C1470.config.signFiles#Null:C1517) && (Value type:C1509(This:C1470.config.signFiles)=Is collection:K8:32))
		...
		For each ($signFilePath; This:C1470.config.signFiles)
			// Resolve file path
			$signFile:=File:C1566(Replace string:C233($signFilePath; "\\"; "/"))
			...
			If (Not:C34($signFile.exists))
				If (This:C1470.config.workingDirectory#Null:C1517)
					$signFile:=Folder:C1567(This:C1470.config.workingDirectory).file($signFilePath)
				End if 
			End if 
			
			If ($signFile.exists)
				var $fileCmd : Text
				$fileCmd:=$cmd+" \""+$signFile.path+"\""
				Storage:C1525.github.debug("Signing file: "+$fileCmd)
				
				// Execute sign command
				var $worker : 4D:C1709.SystemWorker
				$worker:=4D:C1709.SystemWorker.new($fileCmd).wait()
	...
	// Sign main executable files in base folder
	var $executableFiles : Collection
	$executableFiles:=$baseFolder.files(fk recursive:K87:7).query("extension = .exe OR extension = .dll")
	
	If ($executableFiles.length>0)
		Storage:C1525.github.notice("Signing executable files in "+$baseFolder.path)
		
		var $execFile : 4D:C1709.File
		For each ($execFile; $executableFiles)
			var $execCmd : Text
			$execCmd:=$cmd+" \""+$execFile.path+"\""
			Storage:C1525.github.debug("Signing executable: "+$execCmd)
			
			// Execute sign command
			var $execWorker : 4D:C1709.SystemWorker
			$execWorker:=4D:C1709.SystemWorker.new($execCmd).wait()
			
			If (($execWorker.response#Null:C1517) && (Length:C16($execWorker.response)>0))
				Storage:C1525.github.info($execWorker.response)
			End if 
			If (($execWorker.responseError#Null:C1517) && (Length:C16($execWorker.responseError)>0))
				Storage:C1525.github.warning($execWorker.responseError)
			End if 
			
			var $execStatusFile : Object
			$execStatusFile:=New object:C1471("success"; $execWorker.exitCode=0; "errors"; $execWorker.errors; "exitCode"; $execWorker.exitCode)
			Storage:C1525.github.debug(JSON Stringify:C1217($execStatusFile))
			
			This:C1470._mergeResult($status; $execStatusFile)
		End for each 
	Else 
		Storage:C1525.github.warning("No executable files found to sign in "+$baseFolder.path)
	End if 
...
Function _getCertificateInfo() : Object
	...
	Case of 
		...
		: (Length:C16(String:C10(This:C1470.config.signCertificatePath))>0)
			Storage:C1525.github.warning("Certificate path provided but identity extraction not implemented")
